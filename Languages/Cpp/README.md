...
- [맨처음 C++은 객체지향을 위해 태어난 언어](#맨처음-C++은-객체지향을-위해-태어난-언어)
...
하지만 현재의 모던 C++로 넘어오면서 C++은 다중 패러다임의 언어로 바뀌었다. 절차지향, 객체지향, 제너릭, 메타 프로그래밍, 함수형까지 거의 모든 페러다임의 연합체라고 볼 수 있을 정도로 거대해져 버렸다.
...
선언 및 정의는 비슷한 것으로 얼핏 당연한 기초개념으로 중요하게 여겨지지 않는 경우가 사람들 사이에서 더럿 있다. 하지만 이는 매우 큰 오산이다. 선언 및 정의에 대한 개념 미숙으로 큰 프로젝트에서 엄청난 곤욕을 맞보는 경우가 허다하다. 선언과 정의는 엄연히 완전히 다른 개념이고 구분되어야만 한다. 사람들이 이렇게 선언 과 정의를 헷갈려하는 이유는 다음과 같다.

> *'`선언`은 여러번 가능 하지만 `정의`는 오로지 한 번만 되어야 한다.'*
...
느슨한 언어를 주로 다룬 사람들은 위의 코드에서 무엇이 잘못되었는지 모를 것이다. 하지만 다소 엄격한 문법을 요구하는 C/C++의 경우, 위와 같은 코드는 애초에 컴파일조차 되지 않는다. 이유는 다음과 같다.

> 컴파일러는 위에서부터 차례대로 코드를 해석한다. 때문에 main() 안에서 add(1, 2) 를 만난 시점에서는 해당 함수에 대해 전혀 알지 못한다.

여기서 간과해서는 안되는 사실은 '<u>엄밀한 의미로 컴파일 과정은 링킹과 구별된다</u>'는 것이다. 참고로 컴파일러는 목적파일을 만들어내고 링커는 하나 이상의 목적파일을 심볼테이블을 이용해서 하나의 실행가능한 실행파일을 만들어낸다 (보통 이러한 링킹 과정을 빌드라고 부른다). 여기서 중요한 것이 바로 `심볼 테이블`인데 심볼 테이블에는 하나의 함수가 호출될 때 필요한 메모리 크기를 계산해둔 값이 저장되어있다. 컴파일 과정에서 코드 세그먼트에 생성이 된다.

즉, add()를 만난 시점에서 컴파일러는 add라는 함수에 대해서 알고있어야 로직 중간중간의 필요 메모리를 계산할 수 있지만 해당 시점에선 아예 add를 알 수 없기에 컴파일 자체가 되지 않는 것이다. 때문에 다음과 같이 해주어야 한다.

```cpp
#include <iostream>
using namespace std;
// 파라미터는 타입만 있어도 되지만, 그 의도를 알기 위해 변수명을 같이 써주는 것이 좋음.
// 이제 심볼테이블에 add가 추가되었을 것이다.
int add(int a, int b); 
// main에 대해서 대략적인 사항을 기록해야한다.
int main()
{
    // 컴파일러는 add의 존재에 대해서 알고있다.
    cout << add(1, 2) << endl;
	return 0;
}
// add의 세부 사항에 대해 추가로 기록할 것이다.
int add(int a, int b)
{
    return a + b;
}
...
또는 정의 부분을 main위로 올려도 된다. 왜냐면 C++에서 정의를 하면 선언도 포함이 되기 때문이다.

큰 프로젝트를 수행하기 위해 좀 더 심화적인 내용을 다뤄보자. [전방 선언(예정)](#전방-선언)은 뭘까? 컴파일러에 대해 더 깊이 알아보자.
