...
프로그래밍을 하다보면 `cosnt`라는 녀석이 괴롭히는 경우가 심심치 않게 발생한다. 반환형이 `const`가 아니라는 이유로 컴파일이 안되기도 하고, 여튼 컴파일 오류에서 많은 경우 `const`가 등장한다. 앞으로도 그럴 것이다. 하지만 그럴 때마다 `const`에 겁먹지 않고 손쉽게 문제를 해결할 수 있길 바란다.
